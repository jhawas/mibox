                ...
                <div class="tile-body">
                    <b-container fluid>
                      ...
                      <b-row>
                        ...
                        <b-col md="6" class="my-1" v-if="hasAccess('create-room')">
                      ...
                      >
                        ...
                        <template slot="type_of_room" slot-scope="row">
                            {{row.value.name}}
                        </template>
                        ...
                        <template slot="actions" slot-scope="row" right>
                            <b-button v-if="hasAccess('view-room')">
                                <i class="fa fa-eye" @click="show(row.item)" aria-hidden="true"></i>
                            </b-button>
                            
                            <b-button @click="update(row.item)" v-if="hasAccess('update-room')">
                                <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                            </b-button>
                            <b-button @click="destroy(row.item)" v-if="hasAccess('delete-room')">
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.totalRows = this.allRooms.length;

        },

        computed: {

          ...mapGetters(['allRooms', 'hasAccess']),
        ...
        methods: {
          ...
          update(item) {
            ...
            this.$router.push({ name: 'room-create', params: { id: item.id } });

          },

          show(item) {

            this.$router.push({ name: 'room-show', params: { id: item.id } });
          ...
          destroy(item) {
            ...
            .then((willDelete) => {
              ...
              if (willDelete) {
                  ...
                  const response = this.deleteRoom(item.id);

                  response.then( response => {
                      console.log(response.data);
                      if(response.data.message === 'success') {

                          toastr.success('Room successfully deleted.', 'Message');
